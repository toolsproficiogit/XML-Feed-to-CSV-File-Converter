...
const parseNumericValue = (val: string): number => {
  if (!val) return NaN;
  // First, remove % signs if present, as they mess up the regex sometimes or we want them gone
  ...
  return new Promise((resolve, reject) => {
    ...
    parser.onclosetag = (tagName) => {
      ...
      if (tagName === rootItemTag) {
        ...
        // 1. Apply Filters with Logic (AND/OR)
        let satisfiesFilters = true;
        if (filters.length > 0) {
            // Check first filter
            satisfiesFilters = checkFilter(currentItem[filters[0].path], filters[0]);
            
            // Check subsequent filters with their logic operators
            for (let i = 1; i < filters.length; i++) {
                const f = filters[i];
                const result = checkFilter(currentItem[f.path], f);
                
                if (f.logic === 'OR') {
                    satisfiesFilters = satisfiesFilters || result;
                } else {
                    // Default to AND
                    satisfiesFilters = satisfiesFilters && result;
                }
