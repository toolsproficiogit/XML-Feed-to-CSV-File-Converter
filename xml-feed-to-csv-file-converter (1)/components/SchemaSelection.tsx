...
export const SchemaSelection: React.FC<Props> = ({ 
  ...
  const [newFilterLogic, setNewFilterLogic] = useState<'AND' | 'OR'>('AND');
  ...
  const addFilter = () => {
    ...
    const newFilter: Filter = {
      ...
      logic: filters.length > 0 ? newFilterLogic : undefined // Only apply logic for 2nd+ filter
    };
            ...
            <div className="flex flex-col gap-3 mb-4 flex-1">
               {filters.map((filter, index) => (
                 <div key={filter.id} className="flex items-center gap-2 text-sm bg-[#E6F0EE] p-2 rounded border border-[#083027]/10">
                   {index > 0 && (
                     <span className="bg-[#083027] text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                       {filter.logic}
                     </span>
                   )}
              ...
              <div className="sm:col-span-2 flex gap-2 items-end">
                {filters.length > 0 && (
                   <div className="w-20 shrink-0">
                      <label className="text-[10px] font-bold text-[#083027]/60 uppercase mb-1 block">Logic</label>
                      <select
                        className="w-full text-xs p-1.5 rounded border border-[#083027]/20 bg-white outline-none focus:border-[#F54A23] text-[#083027] font-bold"
                        value={newFilterLogic}
                        onChange={(e) => setNewFilterLogic(e.target.value as 'AND' | 'OR')}
                      >
